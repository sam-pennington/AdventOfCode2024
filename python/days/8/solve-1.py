...
def main() -> None:
    grid: list[list[str]] = load('puzzle-input-1.txt')
    ...
    for _, positions in antenna.items():
        ...
        for idx, position in enumerate(positions):
            for next in range(idx + 1, num_positions):
                ...
                if in_bounds(extents, anti_start): antinodes.add(anti_start)
                if in_bounds(extents, anti_end): antinodes.add(anti_end)
